# Code taken from http://pixelenvy.ca/wa/ca_cave.html
...
class CA_CaveFactory:
	...
	def __set_border(self):
		for j in range(0,self.__length):
			...
			self.map[j][self.__width-1] = PERM_WALL

		for j in range(0,self.__width):
			self.map[0][j] = PERM_WALL
			self.map[self.__length-1][j] = PERM_WALL
	...
	def __stop_drawing(self,pt,npt,cpt):
		if self.__ds.find(npt) == self.__ds.find(cpt):
			return 1
		if self.__ds.find(pt) != self.__ds.find(npt) and self.map[npt[0]][npt[1]] == FLOOR:
			return 1
		else:
			return 0
